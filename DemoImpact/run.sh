...
IC_PATH="$HOME/Desktop/swift-cmgi/DemoImpactInitCond/demo_impact_${N_label}.hdf5"
...
then
    if [ -e "$IC_PATH" ]
    then
        echo "Copying initial conditions from DemoImpactInitCond..."
        cp "$IC_PATH" ./
        echo "Successfully copied demo_impact_${N_label}.hdf5"
    else
        echo "ERROR: Initial conditions not found at $IC_PATH"
        echo "Please make sure DemoImpactInitCond has been run successfully"
